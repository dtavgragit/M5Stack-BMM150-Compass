...
/* =================== (Opcional) compás triangular original =================== */
...
/* =================== Compás avanzado (sin cola gris; grados solo) =================== */
static inline void draw_compass_advanced(float head_dir_deg) {
  const int CX = 160, CY = 120;

  // Geometría de la esfera
  const int R_RING   = 96;   // borde blanco
  const int R_FACE   = 86;   // cara blanca interior
  const int R_NUM    = 108;  // radio para números (fuera del anillo)

  // Aguja (solo punta roja)
  const int R_TIP    = 74;   // punta
  const int R_BASE   = 46;   // centro base de la punta
  const int BASE_HALF= 12;   // semiancho base

  // Fondo
  img.fillSprite(TFT_BLACK);

  // Anillo + cara
  img.drawCircle(CX, CY, R_RING, TFT_WHITE);
  img.fillCircle(CX, CY, R_FACE, TFT_WHITE);
  img.fillCircle(CX, CY, 1, TFT_GREEN);

  // Escala: ticks cada 10°, más largos cada 30° y más largos cada 90°
  for(int d=0; d<360; d+=10){
    float rad = (90.0f - d) * DEG_TO_RAD;
    float c = cosf(rad), s = sinf(rad);

    int len = 6;                 // tick 10°
    if(d % 90 == 0)      len = 16;  // 90°
    else if(d % 30 == 0) len = 10;  // 30°

    int x1 = CX + (int)((R_FACE + 3) * c);
    int y1 = CY - (int)((R_FACE + 3) * s);
    int x2 = CX + (int)((R_FACE + 3 + len) * c);
    int y2 = CY - (int)((R_FACE + 3 + len) * s);
    img.drawLine(x1, y1, x2, y2, TFT_BLACK);
  }

  // Números cada 30° alrededor
  img.setTextColor(TFT_WHITE, TFT_BLACK);
  for(int d=0; d<360; d+=30){
    float rad_txt = (90.0f - d) * DEG_TO_RAD;
    int tx = CX + (int)(R_NUM * cosf(rad_txt));
    int ty = CY - (int)(R_NUM * sinf(rad_txt));
    img.drawCentreString(String(d), tx, ty-10, 2);
  }

  // Cardinales en verde
  img.setTextColor(TFT_GREEN, TFT_BLACK);
  img.drawCentreString("N", CX, CY - R_NUM + 2, 4);
  img.drawCentreString("S", CX, CY + R_NUM - 24, 4);
  img.drawCentreString("O", CX - R_NUM + 6, CY - 12, 4);
  img.drawCentreString("E", CX + R_NUM - 6, CY - 12, 4);

  // AGUJA: solo punta roja con contorno negro
  float radH = (90.0f - head_dir_deg) * DEG_TO_RAD;
  float cH = cosf(radH), sH = sinf(radH);

  int x_tip    = CX + (int)(R_TIP  * cH);
  int y_tip    = CY - (int)(R_TIP  * sH);
  int x_base_c = CX + (int)(R_BASE * cH);
  int y_base_c = CY - (int)(R_BASE * sH);
  int x_bl = x_base_c + (int)( BASE_HALF * sH);
  int y_bl = y_base_c + (int)( BASE_HALF * cH);
  int x_br = x_base_c - (int)( BASE_HALF * sH);
  int y_br = y_base_c - (int)( BASE_HALF * cH);

  img.fillTriangle(x_tip, y_tip, x_bl, y_bl, x_br, y_br, TFT_RED);
  img.drawTriangle(x_tip, y_tip, x_bl, y_bl, x_br, y_br, TFT_BLACK);

  // Lectura inferior: SOLO grados
  img.setTextColor(TFT_RED, TFT_BLACK);
  char buff[16];
  snprintf(buff, sizeof(buff), "%.1f°", head_dir_deg);
  img.drawFastHLine(10, 205, 300, TFT_DARKGREY);
  img.drawCentreString(buff, CX, 210, 4);
...
void loop() {
  ...
  // Usa el compás avanzado sin cola y con grados solamente
  draw_compass_advanced(head_dir);
